...
def get_wan_dimension(config, pipeline, config_key: str, pipeline_path: str = None, default_value: Any = None) -> Any:
  """
...
def get_flux_dimension(config, pipeline, config_key: str, pipeline_path: str = None, default_value: Any = None) -> Any:
  """
...
def log_synthetic_config(
    model_name: str, dimensions: Dict[str, Any], per_host_batch_size: int, is_training: bool, num_samples: Optional[int]
):
...
def _generate_wan_sample(rng_key: jax.Array, dimensions: Dict[str, Any], is_training: bool) -> Dict[str, np.ndarray]:
  ...
  encoder_hidden_states_shape = (per_host_batch_size, dimensions["max_sequence_length"], dimensions["text_embed_dim"])
...
def _make_wan_synthetic_iterator(config, mesh, global_batch_size, pipeline, is_training, num_samples):
  ...
  height = get_wan_dimension(
      config, pipeline, "height", pipeline_path=None, default_value=480  # Not in pipeline, use config/override
  )
  width = get_wan_dimension(
      config, pipeline, "width", pipeline_path=None, default_value=832  # Not in pipeline, use config/override
  )
  num_frames = get_wan_dimension(
      config, pipeline, "num_frames", pipeline_path=None, default_value=81  # Not in pipeline, use config/override
  )

  # WAN-specific dimensions from transformer config
  max_sequence_length = get_wan_dimension(
      config, pipeline, "max_sequence_length", pipeline_path="transformer.config.rope_max_seq_len", default_value=512
  )
  text_embed_dim = get_wan_dimension(
      config, pipeline, "text_embed_dim", pipeline_path="transformer.config.text_dim", default_value=4096
  )
  ...
  vae_scale_factor_spatial = get_wan_dimension(
      config, pipeline, "vae_scale_factor_spatial", pipeline_path="vae_scale_factor_spatial", default_value=8
  )
  vae_scale_factor_temporal = get_wan_dimension(
      config, pipeline, "vae_scale_factor_temporal", pipeline_path="vae_scale_factor_temporal", default_value=4
  )
  ...
  num_train_timesteps = get_wan_dimension(
      config, pipeline, "num_train_timesteps", pipeline_path="scheduler.config.num_train_timesteps", default_value=1000
  )
...
def _make_flux_synthetic_iterator(config, mesh, global_batch_size, pipeline, is_training, num_samples):
  ...
  resolution = get_flux_dimension(
      config, pipeline, "resolution", pipeline_path=None, default_value=512  # Not in pipeline, use config
  )
  max_sequence_length = get_flux_dimension(
      config, pipeline, "max_sequence_length", pipeline_path=None, default_value=512  # Not in pipeline, use config
  )
  ...
  vae_scale_factor = get_flux_dimension(
      config, pipeline, "vae_scale_factor", pipeline_path="vae_scale_factor", default_value=8  # Direct pipeline attribute
  )
