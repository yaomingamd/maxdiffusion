...
def load_sharded_checkpoint(pretrained_model_name_or_path, subfolder, device, filename=None):
  ...
  for index_file in index_candidates:
    candidate = (
        os.path.join(pretrained_model_name_or_path, subfolder, index_file)
        if is_local
        else None
    )
  ...
  if index_path is not None:
      ...
      if not is_local:
        try:
          ckpt_path = hf_hub_download(
              pretrained_model_name_or_path, subfolder=subfolder, filename=candidate_name
          )
          filename = candidate_name
          break
        except EntryNotFoundError:
    ...
    if ckpt_path is None:
      raise FileNotFoundError(
          f"No Ideogram checkpoint found under {pretrained_model_name_or_path}/{subfolder}"
      )
