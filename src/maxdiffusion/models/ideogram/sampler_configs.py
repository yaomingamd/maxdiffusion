...
from maxdiffusion.models.ideogram.scheduler import SamplerParameters
...
def get_sampler_preset(name: str) -> SamplerParameters:
  if name not in PRESETS:
    raise ValueError(f"Unknown Ideogram sampler preset {name!r}. Available: {sorted(PRESETS)}")
  return PRESETS[name]


def guidance_schedule_step_order(preset: SamplerParameters) -> tuple[float, ...]:
  """Return guidance weights indexed by fori_loop step (0 = first denoise step)."""
  gs = preset.guidance_schedule
  n = preset.num_steps
  return tuple(gs[n - 1 - i] for i in range(n))
